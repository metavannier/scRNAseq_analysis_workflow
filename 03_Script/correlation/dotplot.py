## This code come from the workflow tapenade : 
# https://github.com/GuignardLab/tapenade/blob/main/src/tapenade/analysis/spatial_correlation/_spatial_correlation_plotter.py
...
class SpatialCorrelationPlotter:
    ...
    def _get_custom_cmap(self, colormap: str):
        ...
        inferno_cmap = matplotlib.colormaps.get_cmap(colormap)
    ...
    def _format_figure(
        ...
        ax.set_xlabel(gene_X, fontsize=12)
        ax.set_ylabel(gene_Y, fontsize=12)
        plt.title(f"Scatter plot of {gene_X} vs {gene_Y} fot time point {time}")
    ...
    def plot_heatmap(
        ...
        extent_X: tuple = None,
        extent_Y: tuple = None,
        percentiles_X: tuple = (0, 100),  # Add this parameter
        percentiles_Y: tuple = (0, 100),  # Add this parameter
        log_scale_X: bool = False,
        log_scale_Y: bool = False,
        display_quadrants: bool = False,
    ):
...
# Load the data
data_path = snakemake.params.input_count_matrices
data = pd.read_csv(data_path, index_col=0)

# Transpose the data to have cells as rows and genes as columns
data_transposed = data.T

# Select two genes (or any two quantities) for correlation
gene_X = snakemake.params.gene_X 
gene_Y = snakemake.params.gene_Y 

# Select the sample time to analyse
time = snakemake.params.time 

# Extract values for the selected genes
quantity_X = data_transposed[gene_X].values
quantity_Y = data_transposed[gene_Y].values

# Reverse log2 transformation (undo log2 normalization)
quantity_X = np.power(2, quantity_X)  # Apply 2^x to reverse log2 transformation
quantity_Y = np.power(2, quantity_Y)  # Apply 2^x to reverse log2 transformation

# Optionally, define a mask or labels (if needed)
mask = None  # Replace with your mask array if required
labels = None  # Replace with your labels array if required
...
fig, ax = plotter.get_heatmap_figure(
    ...
    extent_X=None,  # Set a valid range or keep it as None
    extent_Y=None,  # Set a valid range or keep it as None
    percentiles_X=(0, 100),  # Correct argument name
    percentiles_Y=(0, 100),  # Correct argument name
    log_scale_X=False,
    log_scale_Y=False,
...
output_file = f'05_Output/05_correlation/Scatter_plot_{gene_X}_{gene_Y}_{time}.png'  # Modify this path
fig.savefig(output_file, dpi=300, bbox_inches='tight')

# Optionally, close the figure to free memory if you don't want it displayed
plt.close(fig)

# Path to the output file
output_file = '05_Output/05_correlation/dotplot_output.txt'

# Message to indicate the step is finished
message = "Step finished: Dot plot generation completed successfully.\n"

# Open the file in append mode to add the message without overwriting the existing content
