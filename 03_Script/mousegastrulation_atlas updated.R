...
# sims_count_atlas <- logNormCounts(sce, size.factors = sizeFactors(sce))
...
while(i <= len)
{
    ...
    sims_count_data <- read.delim(file.path( OUTPUTDIR, STEP2, SAMPLE_ID[i], paste0( SAMPLE_ID[i], "_scran_normalized_matrix.csv")), header=TRUE, row.names=1,sep = ",", check.names=FALSE)
    ...
    # ## Remove the Cells qualified as unknown after the annotation of the atlas by sims :
    # metadata = read.delim(file.path( OUTPUTDIR, STEP3, "mousegastrulation", "metadata_updated.csv"), header=TRUE, row.names=1,sep = ",", check.names=FALSE)
    # # Filter sims_count_atlas_matrix based on matching values in the first column
    # sims_count_atlas_matrix=read.delim(file.path( OUTPUTDIR, STEP3, "mousegastrulation", "mousegastrulation.csv"), header=TRUE, sep = ",", check.names=FALSE)
    # colnames(sims_count_atlas_matrix)[1] <- "Cell"
    # filtered_sims_count_atlas_matrix <- sims_count_atlas_matrix %>%
    #     filter(sims_count_atlas_matrix[,1] %in% metadata[,1])
    # # Save the atlas normalize matrix as csv file    
    dir.create(file.path(OUTPUTDIR, STEP3, SAMPLE_ID[i]))
    # fwrite(x = filtered_sims_count_atlas_matrix , file = file.path( OUTPUTDIR, STEP3, SAMPLE_ID[i], paste0( OUTPUT_NAME_REF_MATRIX,"2.csv")), row.names = TRUE)
    # # rm(filtered_sims_count_atlas_matrix)
    ...
    sims_count_data <- as.matrix(sims_count_data)
