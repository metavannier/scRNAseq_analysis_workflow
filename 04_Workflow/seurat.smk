...
rule seurat_labelTransfert:
    ...
    shell:
        """
        Rscript -e 'if (!requireNamespace("DoubletFinder", quietly=TRUE)) {{
                        if (!requireNamespace("remotes", quietly=TRUE)) install.packages("remotes");
                        remotes::install_github("chris-mcginnis-ucsf/DoubletFinder")
                    }}'
        Rscript {params.scriptdir}/labelTransfert_seurat.R {params.sample_id}
        touch {output.seurat_labelTransfert_output}
        """
