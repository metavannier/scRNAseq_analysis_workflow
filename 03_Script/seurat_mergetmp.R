...
# If there is no merging to do (One unique sample)
if (MERGE == "FALSE"){
...
# If merging multiple Seurat Objects
} else if (MERGE == "TRUE"){
    # For each cluster to merge before the normalization
    smerge_list = as.list(strsplit(SMERGE, ",")[[1]])
    nvector = c(smerge_list[1])
    for (i in 1:length(smerge_list)) {
        sc_sample <- smerge_list[i]
        sc_data_path = paste("05_Output/01_cellranger/", sc_sample,"/outs/filtered_feature_bc_matrix/", sep="")
        # Load the single cell dataset
        sc_data <- Read10X(data.dir = sc_data_path)
        # Initialize the Seurat object with the raw (non-normalized data).
        # LA VOIR COMMENT NOMMER LA VARIABLE DANS LA BOUCLE POUR RETENIR NOM DE L'OBJET
        obj_seurat <- CreateSeuratObject(counts = sc_data, project = sc_sample, min.cells = MINCELLS, min.features = MINFEATURES)
        ## LA VOIR SI MARCHE
        assign(nvector,obj_seurat)
        nvector <- append(nvector, obj_seurat)
    }
    print(nvector)
    obj_seurat <- merge(smerge_list[1], y = nvector[3:length(smerge_list)-1], add.cell.ids = c("M12","M15","M27","Normal_Melanocytes"), project = "SRP287614")
